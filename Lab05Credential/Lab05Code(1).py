...
def credential_Issuing(params, pub, ciphertext, issuer_params):
    ...
    #I love python <3
    variables = ["x0", "x0_bar", "x1", "beta", "x1b", "r_prime"] 
    ws = {key: o.random() for key in variables}
    
    WX1 = ws['x1'] * h
    WX1b = ws['beta'] * X1
    Wx1b = ws['x1b'] * h
    Wu = ws['beta'] * g
    Wna = ws['r_prime'] * g + ws['x1b'] * a
    Wnb = ws['r_prime'] * pub + ws['x1b'] * b + ws['x0'] * u 
    WCx0 = ws['x0'] * g + ws['x0_bar'] * h
    ...
    rs[1] = ws['beta'] - c*_b
    rs[2] = ws['x1b'] - c*x1b
    rs[3] = ws['r_prime'] - c*r_prime
    rs[4] = ws['x0'] - c*x0
    rs[5] = ws['x0_bar'] - c*x0_bar    
